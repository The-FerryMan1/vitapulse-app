...
const setUpWebSocketConnection = () => {
    ws = new WebSocket('ws://localhost:8888/api/ws');
    ws.onopen = (event) => {
    ...
    <userLayout>
        <UContainer class="py-4 px-2 w-full mt-4">
            ...
            <div v-if="data?.diastolic && data?.systolic && data?.pulseRate && user?.birthday"
                ...
                <AsyncPulseRateCard
                    :PulseRate="{ pulse_rate: data.pulseRate, age: Number(getTheAge(String(user?.birthday))) }" />
            </div>
            <div v-else class="p-3 my-2 bg-amber-500 text-black rounded-sm shadow-sm flex items-center gap-5">

                <div class="flex flex-col">

                    <p class="text-lg font-semibold flex gap-3 items-center">
                        <UIcon name="i-lucide-unplug" class="size-5" />Real-time connection lost
                    </p>
                ...
                <UButton @click="reconnect" class="bg-amber-300 ms-auto text-black" icon="i-lucide-plug-zap">Reconnect
                </UButton>
            </div>
        </UContainer>
